...
def load_category_overrides(path: Path) -> dict[str, str]:
    """Load category_override.yaml → flat dict {normalized_canonical: category}."""
...
def assign_category(category_distribution: dict[str, int]) -> tuple[str, bool]:
    """
    Assign 1 category to a cluster by dominant vote.
    Returns (category, tie_flag) where tie_flag=True if top 2 counts are equal.
    """
    if not category_distribution:
        return ("Other", False)
    sorted_cats = sorted(category_distribution.items(), key=lambda x: x[1], reverse=True)
    top_cat, top_count = sorted_cats[0]
    tie = len(sorted_cats) > 1 and sorted_cats[1][1] == top_count
    return (top_cat, tie)
...
def run_pipeline(
    ...
    category_override_path: Path | None = None,
) -> None:
    t_start = time.time()
    ...
    for cid, grp in tqdm(agg_df.groupby("cluster_id", sort=False), total=n_clusters, desc="  build clusters", unit="cluster", leave=False):
        ...
        dominant_category, tie_flag = assign_category(cat_dist)
    ...
    # -----------------------------------------------------------------------
    # §-- Category overrides (cross-category tie resolution)
    # -----------------------------------------------------------------------
    category_override_stats: dict[str, Any] | None = None
    if category_override_path is not None:
        cat_override_map = load_category_overrides(category_override_path)
        logger.info("Loaded %d category overrides from %s", len(cat_override_map), category_override_path)

        n_overridden = 0
        for idx, row in clusters_df_out.iterrows():
            if row["applied_other_bucket"]:
                continue  # other-bucket final_canonical already bakes in old dominant_category
            key = str(row["final_canonical"]).lower().strip()
            if key in cat_override_map:
                clusters_df_out.at[idx, "dominant_category"] = cat_override_map[key]
                clusters_df_out.at[idx, "tie_flag"] = False  # tie resolved by override
                n_overridden += 1

        category_override_stats = {
            "category_override_file": str(category_override_path),
            "n_override_entries": len(cat_override_map),
            "n_clusters_overridden": n_overridden,
        }

        print("\n" + "=" * 60)
        print("CATEGORY OVERRIDES")
        print("=" * 60)
        print(f"  Loaded : {len(cat_override_map)} entries from {category_override_path.name}")
        print(f"  Applied: {n_overridden} clusters updated")
        print("=" * 60 + "\n")
        logger.info("Category overrides: %d clusters updated.", n_overridden)
    ...
    cross_df = cross_df.sort_values("total_count", ascending=False).reset_index(drop=True)

    # -----------------------------------------------------------------------
    # §7.6  Cross-category summary for console & report
    # -----------------------------------------------------------------------
    n_cross_ties = int(cross_df["tie_flag"].sum()) if "tie_flag" in cross_df.columns else 0
    cross_warning = ""
    if n_cross_ties > 0:
        cross_warning = (
            f"{n_cross_ties} cross-category cluster(s) have a tie — "
            "manual tie-break required. See cross_category_ties.csv."
        )
    ...
    print(f"  -> All resolved by dominant-vote (no 4-branch action)")
    print(f"  -> Ties requiring manual review : {n_cross_ties}")
    print("=" * 60 + "\n")
    ...
    report: dict[str, Any] = {
        ...
        "linkage": "average",
        "min_keep_count": min_keep_count,
        "cross_category_strategy": "dominant_vote_v1",
        "tie_breaker": "manual",
        "removed_logic": "suggest_action_4_branch",
        "spot_check_threshold_pct": 60,
        "spot_check_sample_size": 30,
        "audit_finding": "no mis-clustering detected; all cross-category traces to LLM category-labeling noise",
        "short_string_bypass": {
        ...
        "category_override": category_override_stats,
    ...
    out_cols_clusters = [
        ...
        "tie_flag", "categories_involved", "category_distribution",
    ...
    # 4b. cross_category_ties.csv — clusters needing manual tie-break
    ties_df = (
        cross_df[
            (cross_df["tie_flag"] == True) &  # noqa: E712
            (cross_df["total_count"] >= min_keep_count)
        ].copy()
        if "tie_flag" in cross_df.columns
        else pd.DataFrame(columns=cross_out_cols)
    )
    ties_df[cross_out_cols].to_csv(output_dir / "cross_category_ties.csv", index=False)
    logger.info("Wrote cross_category_ties.csv (%d rows)", len(ties_df))
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Path to category_override.yaml for manual tie-break resolution of cross-category clusters.",
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_pipeline(
        ...
        category_override_path=args.category_override,
    )
